...
const { flagCodeFor, FLAG_SOURCE } = require("./flags.js");
const { NO_POLYGON, BORDER_SOURCE } = require("./borders.js");
...
// Folded straight into the Geography group (not their own group) — same picker, same Geography
// Challenge pool, same GOAT/leaderboard treatment as the country lists they're drawn from.
const FLAG_CATS = FLAG_SOURCE.map(([baseName, flagName]) => {
  ...
  return { name: flagName, group: "Geography", emoji: base.emoji, entries, isFlagQuiz: true };
});
CATS.push(...FLAG_CATS);

// Borders quizzes: same idea, same source lists, answered by recognizing the country's shape —
// played on the real geography map (lib/geo-cats.js routes these names to "map" mode, same as
// their "Countries in ..." counterpart) with one shape highlighted as the target, instead of a
// flag or a name. A couple of countries have no drawable polygon at the atlas's resolution
// (lib/borders.js) and are dropped rather than shown as a blank shape.
const BORDER_CATS = BORDER_SOURCE.map(([baseName, borderName]) => {
  const base = CATS.find((c) => c.name === baseName);
  const entries = base.entries.filter((e) => !e.aliases.some((a) => NO_POLYGON.has(a)));
  return { name: borderName, group: "Geography", emoji: base.emoji, entries, isBorderQuiz: true };
});
CATS.push(...BORDER_CATS);
...
// "Has a board" here means what geoBoardCats() means by it, which includes the flag grids —
// hasGeoBoard() alone only knows about the map/fill categories, so flags would sort as
// boardless and the two notions would disagree.
const grank = (c) => (c.group === "Geography" ? (hasGeoBoard(c.name) || c.isFlagQuiz || c.isBorderQuiz ? 0 : 1) : 2);
...
const RECOMMENDED_TIME = {
  ...
  "Borders of the World": 900, "Borders of Europe": 300, "Borders of Asia": 300, "Borders of Africa": 360,
  "Borders of North America": 240, "Borders of South America": 120, "Borders of Oceania": 150,
};
...
// Categories with a geography board, a flags quiz, or a borders quiz — the ones with a
// per-category leaderboard. Borders quizzes are hasGeoBoard()-true already (they get the real
// map now, see lib/geo-cats.js), so only Flags needs adding on top explicitly.
...
//
// This is geoBoardCats(), NOT every Geography category. The whole point of the Geography
// Challenge is that it lands you on a map, a flag grid or a borders quiz; picking the group by
// name also offered Languages of the World, Natural Disasters, Major Rivers, Famous Mountains,
// Deserts, Seas and Oceans, Tourist Attractions in America and Major American Cities — all of
// which are ordinary typing lists with nothing to draw. `nonSprint` isn't applied either: it
// excludes anything under 12 answers, which was quietly dropping Countries in Central America
// (7 countries, and a real map) from a picker where you choose deliberately.
const geoChallengeCats = () => geoBoardCats();

module.exports = {
  CATS, GENRES, GENRE_EMOJI, findCat, buildCat, nonSprint, TROLL,
  RECOMMENDED_TIME, recommendedTime, QUICK_MAX_SECONDS, quickPlayPool,
  shuffle, genreRoundLimit, pickGenreRounds, geoBoardCats, geoChallengeCats,
  FLAG_CATS, BORDER_CATS,
};

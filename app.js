// Prove It! · single-page app: multiplayer + solo share one document.
// Each module below runs in its own IIFE, so their identically-named helpers
// (show, setTheme, norm, $, VISITOR_ID, setAppHeight, …) stay isolated and
// never collide. The router toggles the multiplayer screens (#home/#room/#game)
...
function updateOnline() {
  ...
  const inSolo = !$("soloApp").classList.contains("hidden"); // solo overlay covers the lobby
  $("online").classList.toggle("hidden", inGame || inSolo || onlineCount <= 0); // shown on home/lobby only
}
...
if (new URLSearchParams(location.search).get("id")) window.PI.showSoloJoin();

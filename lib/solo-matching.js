...
// punctKey/punctMarks for an entry's aliases, computed once and kept on the entry.
//
// The fallback below used to run punctKey over EVERY alias of EVERY entry on EVERY call — an NFD
// normalize and three regexes per alias, thrown away each time. And it only runs on the MISS path,
// which is the one an attacker chooses. Measured on a 197-entry category: 0.0016ms for a hit and
// 0.146ms for a miss, a 91× penalty for typing garbage, paid by the server on the unauthenticated
// score-verification path and on every race answer. Cached, a miss costs about what a hit costs.
//
// Non-enumerable so the entries still serialize and deep-compare as the plain data they were.
function aliasKeys(e) {
  if (!e._pk) {
    Object.defineProperty(e, "_pk", { value: e.aliases.map(punctKey), enumerable: false, configurable: true });
    Object.defineProperty(e, "_pm", { value: e.aliases.map(punctMarks), enumerable: false, configurable: true });
  }
  return e;
}
...
function findEntry(cat, nq) {
  ...
  if (!key) return null;
  const hits = cat.entries.filter((e) => aliasKeys(e)._pk.includes(key));
  if (hits.length === 1) return hits[0];
  if (!hits.length) return null;
  const marks = punctMarks(nq);
  const exactMarks = hits.filter((e) => e._pk.some((k, i) => k === key && e._pm[i] === marks));

...
// The seat token the server hands back when we take a seat. It is what proves the seat is ours
// when we reconnect — a playerId alone used to be enough, and roomState broadcasts every playerId
// to everyone in the room, so anyone watching could claim someone else's seat.
//
// Session-scoped like `pid`, and for the same reason: a refresh keeps the seat, a new tab does not
// try to take one that is already occupied. Keyed by room so holding a seat in one room never
// looks like a claim on another.
export const getSeatToken = (code) => ss((s) => s.getItem("seat:" + String(code || "").toUpperCase()));
export const setSeatToken = (code, token) =>
  ss((s) => {
    const k = "seat:" + String(code || "").toUpperCase();
    return token ? s.setItem(k, token) : s.removeItem(k);
  });

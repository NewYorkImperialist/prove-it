...
// hasOwnProperty, not `in`: `in` walks the prototype chain, so {{constructor}}, {{toString}} and
// {{valueOf}} resolved to function source instead of falling through to the literal-token fallback
// the note below promises. Not reachable today — the only template is a static file — but the
// fallback is the whole contract of this function.
...
const siteVars = { // fields shared across every templated page
  ...
  OG_URL: SITE.url,
  // One static hand-made card for every share link and for the site itself. (A generated card per
  // link lived here briefly; it was switched off and has since been removed.) It's the 1200x630
  // box every crawler wants, which is what the width/height tags below describe.

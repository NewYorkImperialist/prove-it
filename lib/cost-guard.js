"use strict";
// Two-tier automatic cost cap: keeps the Fly machine from running away on cost.
// Tier 1 ($4, coldThreshold): make idle machines STOP rather than suspend — drops the RAM snapshot,
// at the price of a full boot (~1-3s) on the next visit and every live room being thrown away rather
// than restored. Requires FLY_API_TOKEN.
...
//
// fly.toml autosuspends idle machines, which is what tier 1 used to be FOR — so tier 1 buys very
// little now and tier 2 is the tier that actually caps a runaway bill.

// Fly cost projection — rough estimates; confirm against current Fly pricing.
// shared-cpu-1x run continuously ≈ $1.94/mo; NA/EU egress ≈ $0.02/GB (the only real variable).
// The compute figure is a CEILING rather than a fixed cost: idle machines autosuspend (see fly.toml)
// and are billed only while awake, so every projection here deliberately over-states compute. Two
// caveats to be honest about: the number is the 256MB price and [[vm]] is now 512mb (≈$3.19), and
// suspended machines still carry a small charge for the stored RAM snapshot. Both cut the same way —
// the ceiling is less conservative than it looks — so they are worth fixing together, deliberately.
// coldThreshold = $ line where idle machines switch from suspending to stopping.
...
// Extrapolate this cycle's egress to a projected month-end bill. Compute is added at its ceiling —
// see the note above on why that is deliberately pessimistic now that idle machines suspend.
function projectCost(bw, now) {
  ...
  const projTotal = FLY_COST.computePerMo + egressProj;            // compute at its ceiling, not its bill
...
function createCostGuard({ analytics, SITE, ownerAction }) {
  ...
  // Buffered in memory and flushed to the DB every 60s (a single instance → safe, cheap). A suspend
  // preserves RAM, so an idle machine going to sleep does not lose the unflushed tally; a tier-1
  // stop, or a restart, loses up to 60s of it — which is a rounding error on a bandwidth counter.
  ...
  // ---- Fly Machines API: flip the app between suspend-when-idle and stop-when-idle at runtime ----
  ...
  //
  // Since fly.toml now autosuspends, tier 1 buys much less than it did: the compute cost it exists to
  // cut is already scaled to zero, and the difference between "suspend" and "stop" is only the RAM
  // snapshot's storage — plus a slower wake, and every live room being thrown away rather than
  // restored. Tier 2 (the budget page) is the tier that actually caps a runaway bill now.
  ...
  async function setColdStart(cold) {
    ...
    for (const m of machines) {
      ...
      for (const svc of cfg.services || []) {
        ...
        // The normal branch has to MIRROR fly.toml, or the guard silently undoes the deploy-time
        // config the first time it trips and clears. It used to hardcode always-on ("off" / 1),
        // which was right when that was the default and is wrong now that fly.toml autosuspends —
        // one new billing month would have quietly put the machine back on the clock.
        svc.autostop = cold ? "stop" : "suspend";
        svc.autostart = true;
  ...
  async function applyColdStart(cold) {
    ...
    try { await setColdStart(cold); coldError = null; console.log(`💸 cost guard: idle machines set to ${cold ? "stop (cold starts, rooms lost)" : "suspend"}`); }
    catch (e) { coldError = e.message; console.error(`💸 cost guard: failed to set idle machines to ${cold ? "stop" : "suspend"}:`, e.message); }

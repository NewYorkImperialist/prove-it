...
export const dailyLinkUrl = (challengeId) =>
  `${window.location.origin}/challenge.html?id=${challengeId || todaysDailyId()}`;

export function dailyInvite(score, challengeId) {
  const s = score != null && score !== "" && !isNaN(Number(score)) ? `I named ${score} on today's Prove It! daily challenge. ` : "";
  return `${s}Think you can beat me? Play today's daily: ${dailyLinkUrl(challengeId)}`;
}
...
// Returns { ok } or { ok: false, blocked: true } when the name is rejected.
export async function submitDailyResult({ name, run, challengeId, visitorId }) {
  ...
  await postJSON(`/challenge/${id}/result`, {
    name: n,
  ...
  // for the creator), so one rename fixes every board.

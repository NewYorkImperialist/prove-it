...
// Flags quizzes (lib/flags.js) aren't real categories.js entries — they're a client-side layer
// reusing a "Countries in ..." category's entries — but a run still has to name one of them in
// its rounds array, so the server needs to recognize the name too. Mirror the base category's
// size/items/group rather than duplicating them out of categories.js.
for (const [baseName, flagName] of FLAG_SOURCE) {
  CAT_SIZES[flagName] = CAT_SIZES[baseName];
  CAT_ITEMS[flagName] = CAT_ITEMS[baseName];
  CAT_GROUP[flagName] = "Flags";
}
// Everything a round is allowed to name: real categories plus the Flags quizzes. Kept separate
// from ALL_CAT_NAMES (which mirrors categories.js exactly) rather than folding flags into it.
const ALL_ROUND_NAMES = new Set([...ALL_CAT_NAMES, ...FLAG_CAT_NAMES]);

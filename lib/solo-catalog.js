...
const { NO_POLYGON, SILHOUETTE_SOURCE } = require("./silhouettes.js");
...
// Silhouette quizzes: same idea, same source lists, answered by recognizing the country's shape
// (rendered client-side, see lib/browser/silhouette-map.js) instead of its flag or its name.
// A couple of countries have no drawable polygon at the atlas's resolution (lib/silhouettes.js)
// and are dropped rather than shown as a blank tile.
const SILHOUETTE_CATS = SILHOUETTE_SOURCE.map(([baseName, silName]) => {
  const base = CATS.find((c) => c.name === baseName);
  const entries = base.entries.filter((e) => !e.aliases.some((a) => NO_POLYGON.has(a)));
  return { name: silName, group: "Geography", emoji: base.emoji, entries, isSilhouetteQuiz: true };
});
CATS.push(...SILHOUETTE_CATS);
...
const RECOMMENDED_TIME = {
  ...
  "Silhouettes of the World": 900, "Silhouettes of Europe": 300, "Silhouettes of Asia": 300, "Silhouettes of Africa": 360,
  "Silhouettes of North America": 240, "Silhouettes of South America": 120, "Silhouettes of Oceania": 150,
};
...
// Categories with a geography board, a flags quiz, or a silhouette quiz — the ones with a
// per-category leaderboard.
const geoBoardCats = () => [...CATS.filter((c) => hasGeoBoard(c.name)), ...FLAG_CATS, ...SILHOUETTE_CATS].map((c) => c.name);
...
// Silhouette quizzes are folded into Geography now, so they're already part of this pool.

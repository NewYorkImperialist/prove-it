"use strict";
const { GRACE_MS } = require("./match-rules.js");
// Said out loud in the pause banner below, so it has to come from the same place the server counts.
const GRACE_SECS = Math.round(GRACE_MS / 1000);
...
function duelView(gs, { myId, isSpectator, isGhost, iAmHost }) {
  ...
  if (gs.paused && gs.phase !== "matchover") {
    ...
    v.statusText = `Opponent disconnected · waiting up to ${GRACE_SECS}s for them to reconnect…`;
  }
  return v;
}

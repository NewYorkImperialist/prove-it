...
async function setupMap(catName, entries, container) {
  ...
  const h = Math.max(160, Math.round(rect.height || 300) - (boxEntries.length ? 70 : 0));
  ...
  // Microstates with no polygon: a dot at their location if it lands in view, else a box.
  const boxes2 = [];

...
const { BORDER_SOURCE } = require("./borders.js");
...
// Borders quizzes get the same big map as their "Countries in ..." counterpart — one shape
// highlighted at a time (see lib/browser/geomap.js's quiz mode) instead of lit up freely.
const BORDER_MAP_CATS = new Set(BORDER_SOURCE.map(([, borderName]) => borderName));
...
const MAP_ONLY = {
  "Countries in Oceania": new Set(["Australia", "New Zealand", "Papua New Guinea"]),
  "Borders of Oceania": new Set(["Australia", "New Zealand", "Papua New Guinea"]),
};

function geoMode(catName) {
  if (FILL_CATS[catName]) return "fill";
  if (WORLD_CATS.has(catName) || US_CATS.has(catName) || BORDER_MAP_CATS.has(catName)) return "map";
  return null;
}
const hasGeoBoard = (catName) => geoMode(catName) !== null;

module.exports = { WORLD_CATS, US_CATS, FILL_CATS, MAP_ONLY, BORDER_MAP_CATS, geoMode, hasGeoBoard };

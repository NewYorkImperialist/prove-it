...
let quizTargetId = null; // …and its entry id, so a redraw can restore the highlight
// The projection is fitted to the container's measured box, once, at setup. Rotating a phone
// changes that box completely (844x390 <-> 390x844) but the drawn map couldn't know: the SVG
// letterboxes inside its viewBox, so the shapes survive, but they're fitted to the old aspect
// ratio and the dot-vs-box decision (which depends on the height available) is stale. These
// remember enough to redraw, and `litIds` means a redraw restores progress no matter which
// caller drew the board — the solo hook tracks its own named set, the race board doesn't.
let lastMap = null; // { catName, entries, container }
let litIds = new Set();
let sizeObserver = null;
let redrawTimer = null;
// Bumped by every teardown and every fresh setup, so a redraw already awaiting inside setupMap
// can tell that the board it was re-fitting is no longer the current one. clearTimeout can only
// cancel a redraw that hasn't started yet.
...
// A rotation fires a flurry of resize callbacks, and re-projecting is not cheap, so debounce and
// ignore anything that isn't a real change in the box we fitted to. ResizeObserver (not a window
// resize listener) because the board also changes size when the header around it does.
function stopWatchingSize() {
  clearTimeout(redrawTimer);
  redrawTimer = null;
  redrawGeneration++; // invalidates any redraw currently mid-await
  if (sizeObserver) { sizeObserver.disconnect(); sizeObserver = null; }
}
function watchSize(container) {
  if (typeof ResizeObserver !== "function") return; // older browser: the SVG still letterboxes
  let last = container.getBoundingClientRect();
  sizeObserver = new ResizeObserver(() => {
    const box = container.getBoundingClientRect();
    if (Math.abs(box.width - last.width) < 24 && Math.abs(box.height - last.height) < 24) return;
    last = box;
    clearTimeout(redrawTimer);
    redrawTimer = setTimeout(() => { GeoMap.redraw(); }, 180);
  });
...
export const GeoMap = {
  ...
  async setup(catName, entries, container, named) {
    ...
    litIds = new Set();
    stopWatchingSize();
    // Fill mode is a plain CSS grid — it reflows on its own and must NOT be rebuilt, since that
    // would wipe the capitals already typed into it.
    if (geoMode(catName) === "fill") return setupFill(catName, container);
    const r = await setupMap(catName, entries, container);
    lastMap = { catName, entries, container };
  ...
  light(entryId) {
    litIds.add(entryId); // remembered so a re-projection can restore it
    if (!cur) return;
  ...
  // Re-fit the projection to the container's current box, keeping everything already named and
  // the borders-quiz target. Only map mode needs (or gets) this.
  async redraw() {
    if (!lastMap || !cur) return;
    const { catName, entries, container } = lastMap;
    // setupMap replaces the container's contents with a "Loading map…" placeholder before its
    // first await, so a throw part-way leaves the box showing that while `cur` still points at
    // the nodes it just detached — light()/remaining()/toggleRemaining() would then be operating
    // on elements that aren't in the document, and nothing would tell the caller. Drop the stale
    // board instead: a round with no map still plays as a typing round.
    const generation = ++redrawGeneration;
    const mine = lastMap;
    try {
      await setupMap(catName, entries, container);
    } catch {
      if (generation === redrawGeneration) {
        cur = null;
        container.innerHTML = "";
      }
      return;
    }
    // A teardown() or a fresh setup() while we were awaiting: that board is not ours to touch,
    // and re-lighting it would paint the previous category's answers onto the new one.
    if (generation !== redrawGeneration || lastMap !== mine) return;
    for (const id of litIds) GeoMap.light(id);
    if (quizTargetId != null) GeoMap.highlight(quizTargetId);
  },
  ...
  highlight(entryId) {
    ...
    quizTargetId = entryId == null ? null : entryId;

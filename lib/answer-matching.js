...
// Solo/daily mode does its own equivalent client-side (lib/solo-matching.js) since it isn't
// server-validated — these are the server-side twins of that logic. Keep the two norm()s in
// step: they decide whether the same typed answer counts in solo and in a live game.
const CATEGORY_GROUPS = require("../data/categories.js");
const { findEntry } = require("./solo-matching.js");
...
// Deliberately delegates to solo-matching's findEntry rather than repeating the lookup: the two
// modes share category names and leaderboards, so a guess that counts in solo has to count here
// too. (solo-matching is dependency-free, so requiring it costs the server nothing.)
function resolve(cat, value) {
  return findEntry(cat, norm(value));
}

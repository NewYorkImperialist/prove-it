...
const { BORDER_SOURCE, BORDER_CAT_NAMES, NO_POLYGON } = require("./borders.js");
...
// Flags and Borders quizzes (lib/flags.js, lib/borders.js) aren't real categories.js entries —
// they're a client-side layer reusing a "Countries in ..." category's entries — but a run still
// has to name one of them in its rounds array, so the server needs to recognize the name too.
// Mirror the base category's size/items rather than duplicating them out of categories.js, and
// group them as Geography (lib/solo-catalog.js folds them in there too).
...
for (const [baseName, borderName] of BORDER_SOURCE) {
  ...
  CAT_SIZES[borderName] = items.length;
  CAT_ITEMS[borderName] = items;
  CAT_GROUP[borderName] = "Geography";
}
// Everything a round is allowed to name: real categories plus the Flags/Borders quizzes. Kept
// separate from ALL_CAT_NAMES (which mirrors categories.js exactly) rather than folding these in.
const ALL_ROUND_NAMES = new Set([...ALL_CAT_NAMES, ...FLAG_CAT_NAMES, ...BORDER_CAT_NAMES]);

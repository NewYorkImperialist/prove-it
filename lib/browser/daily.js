...
export async function submitDailyResult({ name, run, challengeId, visitorId }) {
  ...
  // unauthenticated yes/no oracle on OWNER_KEY. Still sending it kept the crown key travelling to
  // a route that has no use for it, which is the shape the whole split was meant to end.
  await postJSON("/challenge/rename", { name: n, visitorId, gid: payload?.gid || "" });
